...
:gotAdmin
    ...
    if '%alteryxPath%' NEQ '' (
        ...
        echo Deleted installed config from "%alteryxPath%\..\Settings\AdditionalPlugins\StreamingPlugIn.ini"
    echo Config installed to "%alteryxPath%\..\Settings\AdditionalPlugins\StreamingPlugIn.ini"
    ) else (
        echo Please delete "%cd$\StreamingPlugIn.ini" from <AlteryxInstallDir>\Settings\AdditionalPlugins
    )
